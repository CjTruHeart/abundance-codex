# Domain Registry — The Abundance Codex

> Status dashboard for all 21 Grand Challenge domains. Updated as entries are forged.

---

## Pillar I — The Material Foundation

*Survival first. Everything else follows.*

| # | Domain | Slug | Entries | Status |
|---|--------|------|:-------:|--------|
| 01 | Energy & Power | `energy` | 3 | 🟢 origin_story, breakthrough, false_dawn |
| 02 | Food & Agriculture | `food` | 3 | 🟢 breakthrough, trendline, shadow |
| 03 | Water & Sanitation | `water` | 3 | 🟢 breakthrough, breakthrough, trendline |
| 04 | Shelter & Housing | `shelter` | 3 | 🟢 breakthrough, trendline, false_dawn |
| 05 | Health & Medicine | `health` | 3 | 🟢 breakthrough, trendline, breakthrough |
| 06 | Environment & Climate | `environment` | 3 | 🟢 origin_story, trendline, shadow |

## Pillar II — Human Capability

*From surviving to self-actualizing.*

| # | Domain | Slug | Entries | Status |
|---|--------|------|:-------:|--------|
| 07 | Education & Learning | `education` | 0 | ⬜ Empty |
| 08 | Longevity & Healthspan | `longevity` | 0 | ⬜ Empty |
| 09 | Consciousness & Meaning | `consciousness` | 0 | ⬜ Empty |

## Pillar III — Collective Coordination

*From individuals to federation.*

| # | Domain | Slug | Entries | Status |
|---|--------|------|:-------:|--------|
| 10 | Communication & Connection | `communication` | 0 | ⬜ Empty |
| 11 | Community & Belonging | `community` | 0 | ⬜ Empty |
| 12 | Governance & Institutions | `governance` | 0 | ⬜ Empty |
| 13 | Security & Safety | `security` | 0 | ⬜ Empty |
| 14 | Transportation & Mobility | `transportation` | 0 | ⬜ Empty |
| 15 | Economy & Prosperity | `economy` | 0 | ⬜ Empty |

## Pillar IV — Production & Discovery

*The compounding engine.*

| # | Domain | Slug | Entries | Status |
|---|--------|------|:-------:|--------|
| 16 | Manufacturing & Materials | `manufacturing` | 0 | ⬜ Empty |
| 17 | Computation & AI | `computation-intelligence` | 0 | ⬜ Empty |
| 18 | Co-Evolution & Human-AI Symbiosis | `co-creative-intelligence` | 0 | ⬜ Empty |
| 19 | Discovery & Frontier Science/Engineering | `science-engineering` | 0 | ⬜ Empty |

## Pillar V — The Transcendent Frontier

*Where humanity reaches beyond itself.*

| # | Domain | Slug | Entries | Status |
|---|--------|------|:-------:|--------|
| 20 | Space & Exploration | `space` | 0 | ⬜ Empty |
| 21 | Collective Imagination & Narrative Infrastructure | `future-vision` | 0 | ⬜ Empty |

---

## Totals

| Metric | Count |
|--------|:-----:|
| Total domains | 21 |
| Domains with entries | 6 |
| Total forged entries | 18 |
| Codex version | 1.1 |

---

## Domain Number Note

Domain numbers flow sequentially through the five pillars. A reader walking from 01 to 21 traverses the civilization-building arc from material foundation to transcendent frontier.

```
Pillar I  (Material Foundation)      → 01-06
Pillar II (Human Capability)         → 07-09
Pillar III (Collective Coordination) → 10-15
Pillar IV (Production & Discovery)   → 16-19
Pillar V  (Transcendent Frontier)    → 20-21
```
